...
The circuit editor currently supports:

- Creating custom variants from the shipped circuit catalog.
- Moving, inserting, duplicating, smoothing, and deleting control points.
- Selecting the start/finish point and changing overall circuit width.
- Undo, redo, keyboard shortcuts, zoom, and grid snapping.
- Intersection checks and compiled-track validation.
- Boundary, curvature, and surface-grip analysis overlays.
- Placing, scaling, rotating, locking, and reordering scenery elements.
- Browser-local saving of custom circuits.

The advanced track domain also models starting grids, timing sectors, surface zones, curbs, runoff, barriers, and pit lanes. Not all of those fields have player-facing editor controls yet.
...
- Custom circuit scenery is visible in the editor but is not yet rendered in races.
- The race runtime remigrates the legacy circuit shape instead of consuming every saved advanced track-document field.
- Pit lanes, timing-sector editing, surface-zone editing, and several other advanced track fields are foundations rather than complete features.
