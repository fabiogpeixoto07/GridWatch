...
- Reusable straights, turns, chicanes, route connections, and primary/pit/alternate paths. The module palette presents the complete geometry catalog by category without a search filter. The Freeform tool is a guided End → Start bridge: select two open connectors on the active route and it draws a tangent-aligned road using the selected Start piece's settings; its intentional joins are permitted while unrelated road collisions remain blocked.
...
- Start/Finish direction controls visible clockwise/counter-clockwise travel, the racing line, finish line, grid headings, and physics route. The selected Start/Finish marker is also the physical lap seam, so a car is classified and removed only after passing that authored line. Pole position is held four metres behind that line, keeping the finish marker ahead of the entire grid. Starting-grid slots use traditional three-sided `]` paint marks rather than placeholder rectangles; they remain painted in both the Track Editor and race mode, while their authored positions are compiled directly into the race.
...
- **Convert Image to Layout** traces the dominant connected near-black region of a PNG, JPEG, or WebP into a closed chain of editable Freeform road modules. Non-black regions and isolated speckles are discarded, the source image is retained as an alignment overlay, and Start / Finish plus starting-grid placement remain unset so they can be authored manually afterward.
- `publish.cmd` verifies the locked Node/npm toolchain and now skips repeated Windows feature servicing when IIS is already available, avoiding unnecessary servicing waits while preserving the installation path for new hosts.
