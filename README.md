...
- Start/Finish direction controls visible clockwise/counter-clockwise travel, the racing line, finish line, grid headings, and physics route. The selected Start/Finish marker is also the physical lap seam, so a car is classified and removed only after passing that authored line. Pole position is held four metres behind that line, keeping the finish marker ahead of the entire grid. Starting-grid slots use traditional three-sided `]` paint marks rather than placeholder rectangles, and their authored positions are compiled directly into the race.
