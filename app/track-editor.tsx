...
export function TrackEditor({ tracks: _tracks = [], onSave, onBack }: { tracks?: Circuit[]; onSave: (circuit: EditableCircuit) => void; onBack: () => void }) {
  ...
  const worldPoint = (event: { currentTarget: Element; clientX: number; clientY: number }) => {
    const rect = event.currentTarget.getBoundingClientRect();
    const raw = { x: ((event.clientX - rect.left) / rect.width) * VIEWBOX_WIDTH, y: ((event.clientY - rect.top) / rect.height) * VIEWBOX_HEIGHT };
  ...
  useEffect(() => {
    ...
    const onPointerUp = (event: PointerEvent) => {
      ...
      const point = { x: ((event.clientX - rect.left) / rect.width) * VIEWBOX_WIDTH, y: ((event.clientY - rect.top) / rect.height) * VIEWBOX_HEIGHT };
